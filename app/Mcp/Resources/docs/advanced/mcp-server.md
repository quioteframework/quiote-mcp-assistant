...
The tool handler is either a capability you registered on `McpCatalog`, or — for an action exposed with `#[McpTool]` — the `ActionToolAdapter`, which runs the call through the context's PSR-15 request handler so it gets the *same* DI resolution, verb dispatch, and validation a real HTTP request would (see [the actions-as-tools bridge](#the-killer-feature-mcptool-on-an-existing-action) above). See [Request lifecycle](/architecture/request-lifecycle/) for the full pipeline this plugs into.
