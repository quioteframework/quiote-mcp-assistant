...
| Since | `1.0.0` |
| Source | `Validator/JsonValidator.php` |

## Methods

| Method | Description |
|---|---|
| [`getAcceptedParameters(): array<int, string>`](#getacceptedparameters) | Returns the base Validator parameters plus 'assoc'. |
...
'assoc' is the boolean handed to json_decode(): true, the default, decodes objects into associative arrays, false into stdClass instances. It only affects the shape of the exported decoded value, not whether the input validates. A non-boolean value raises a parameter-type error at validation time.
