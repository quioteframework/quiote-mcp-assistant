...
> IsNotEmptyValidator verifies a parameter is not empty The content of the input value is not verified in any manner, it is only checked if the input value exists and is not empty.

IsNotEmptyValidator verifies a parameter is not empty The content of the input value is not verified in any manner, it is only checked if the input value exists and is not empty.

It lets the data holder implementation decide what is regarded as empty.
...
| Since | `1.0.0` |
| Source | `Validator/IsNotEmptyValidator.php` |
