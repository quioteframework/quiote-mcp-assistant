...
> ArraylengthValidator verifies the length (count()) constraints for an array Parameters: 'min' The array should contain at least 'min' elements 'max' The array should contain at most 'max' elements
...
| Since | `1.0.0` |
| Source | `Validator/ArraylengthValidator.php` |

## Methods

| Method | Description |
|---|---|
| [`getAcceptedParameters(): array<int, string>`](#getacceptedparameters) | Returns the base Validator parameters plus 'min' and 'max'. |
...
'min' and 'max' bound the element count of the array being validated; each is optional and only checked when present.
