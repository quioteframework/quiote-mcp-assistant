...
[`CycleRecordingLogger`](/api/replay/adapter/cycle/cycle-recording-logger/) is installed once, via `Cycle\Database\DatabaseManager::setLogger()`, so this source only has to point [`ActiveEffectLedger`](/api/replay/recording/active-effect-ledger/) at the current request's ledger -- every `ReplayCycleDatabase` connection reads it from there.
