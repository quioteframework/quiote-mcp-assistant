...
> Base class an emitted `--as-test` test extends: `replay()` reconstructs a cassette's request and dispatches it through the same real pipeline HttpTestCase's own `get()`/`post()`/etc.

Base class an emitted `--as-test` test extends: `replay()` reconstructs a cassette's request and dispatches it through the same real pipeline [`HttpTestCase`](/api/testing/http-test-case/)'s own `get()`/`post()`/etc.

helpers use (`getContext()->getRequestHandler()->handle()`), returning the same [`TestResponse`](/api/testing/http/test-response/) the rest of the suite already asserts against.

Deliberately does **not** go through [`ReplayEngine`](/api/replay/replay/replay-engine/): that class exists for the interactive `quiote replay <id>` CLI workflow and enforces `replay.allow_live` -- appropriate to a developer pointing the command at a real, possibly-shared application, and wrong for a committed regression test that must run unattended in CI with nothing configured beyond having the package installed.

Replays in isolation by default ([`IsolatedReplay`](/api/replay/replay/isolated-replay/)), which is what makes an emitted test safe to run unattended: every ledger-backed subsystem is answered from the cassette's own recorded effects and nothing is performed, so a recorded POST or DELETE re-runs on every CI build without re-performing the write. It needs no configuration and no database, which is the promise that made bypassing `ReplayEngine` right in the first place.

`replay.tests_allow_live = true` opts a suite out, into a live dispatch against whatever the test context is really configured with -- for the case a test genuinely needs real collaborators. That is the whole decision: it accepts real reads *and* real writes, on every run, and is only safe where the environment is disposable. The isolated default is the protection; turning it off is choosing not to have it.
...
|  |  |
|---|---|
| Extends | [`HttpTestCase`](/api/testing/http-test-case/) |
| Source | `Testing/ReplayTestCase.php` |

## Methods

| Method | Description |
|---|---|
| [`replay(string $cassettePath): TestResponse`](#replay) | Reconstructs the request `$cassettePath` recorded and dispatches it through the real pipeline, returning the response as a [`TestResponse`](/api/testing/http/test-response/). |

### replay()

`protected function replay(string $cassettePath): TestResponse`

Reconstructs the request `$cassettePath` recorded and dispatches it through the real pipeline, returning the response as a [`TestResponse`](/api/testing/http/test-response/).
...
| Throws | When |
|---|---|
| `CassetteCodecException` | if the file is not a valid cassette. |
| `ReplayException` | if the cassette file cannot be read, carries no replayable request (e.g. a `#[NoRecord]` skeleton), or -- in isolation -- if the registered effect sources cannot serve from the ledger. |
...
| `delete()` | [`HttpTestCase`](/api/testing/http-test-case/) |  |
...
| `fileExists()` | `Assert` |  |
| `get()` | [`HttpTestCase`](/api/testing/http-test-case/) |  |
...
| `isTrue()` | `Assert` |  |
| `isWritable()` | `Assert` |  |
| `json()` | [`HttpTestCase`](/api/testing/http-test-case/) |  |
| `lessThan()` | `Assert` |  |
...
| `objectEquals()` | `Assert` |  |
| `patch()` | [`HttpTestCase`](/api/testing/http-test-case/) |  |
| `post()` | [`HttpTestCase`](/api/testing/http-test-case/) |  |
| `put()` | [`HttpTestCase`](/api/testing/http-test-case/) |  |
