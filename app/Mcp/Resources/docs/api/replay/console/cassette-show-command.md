...
> `cassette:show <id>` -- decodes one cassette and prints the requested projection.

`cassette:show <id>` -- decodes one cassette and prints the requested projection.

Request/response bodies are excerpted (length + sha256 by default) unless `--include-bodies` is passed, so a 2 MiB cassette does not become 2 MiB of terminal output by accident.
...
| Source | `Console/CassetteShowCommand.php` |
