...
> UnitTestCase is the base class for all unit testcases and provides the necessary assertions
...
| Implements | [`IUnitTestCase`](/api/testing/i-unit-test-case/) |
| Since | `1.0.0` |
| Source | `Testing/UnitTestCase.php` |
...
| Method | Description |
|---|---|
| [`assertSessionMechanismConfigured(): void`](#assertsessionmechanismconfigured) | Fail unless this test's context actually has a session mechanism. |
| [`getContext(): Context`](#getcontext) | Return the context defined for this test (or the default one). |
| [`installTestSessionManager(array<string, mixed> $parameters = []): SessionManager`](#installtestsessionmanager) | Install a real [`SessionManager`](/api/session/session-manager/) on this test's context, backed by in-memory persistence. |
| [`installTestTranslationManager(array<string, mixed> $parameters = []): TranslationManager`](#installtesttranslationmanager) | Install a translation manager on this test's context, and answer it. |
| [`newWebRequest(array<string, mixed> $parameters = [], array<string> $additionalWhitelist = []): WebRequest`](#newwebrequest) | Convenience factory for PSR-compatible WebRequest instances in tests. |

### assertSessionMechanismConfigured()

`protected function assertSessionMechanismConfigured(): void`

Fail unless this test's context actually has a session mechanism.

A guard for suites that only mean anything with a session present: without it, losing the session slot (or forgetting [`UnitTestCase::installTestSessionManager()`](/api/testing/unit-test-case/#installtestsessionmanager)) turns the suite green while it silently stops testing its subject.
...
### installTestSessionManager()

`protected function installTestSessionManager(array<string, mixed> $parameters = []): SessionManager`

Install a real [`SessionManager`](/api/session/session-manager/) on this test's context, backed by in-memory persistence.

Passed to the SessionManager constructor
            (`cookie_name`, `session_migration_grace_seconds`, ...). Defaults produce
            the framework's own defaults, i.e. a `QSID` cookie.

| Parameter | Type | Description |
|---|---|---|
| `$parameters` | `array``<``string``, ``mixed``>` | Passed to the SessionManager constructor (`cookie_name`, `session_migration_grace_seconds`, ...). Defaults produce the framework's own defaults, i.e. a `QSID` cookie. |

Returns [`SessionManager`](/api/session/session-manager/) — The installed manager.

### installTestTranslationManager()

`protected function installTestTranslationManager(array<string, mixed> $parameters = []): TranslationManager`

Install a translation manager on this test's context, and answer it.

Passed to the manager's initialize().

| Parameter | Type | Description |
|---|---|---|
| `$parameters` | `array``<``string``, ``mixed``>` | Passed to the manager's initialize(). |

Returns [`TranslationManager`](/api/translation/translation-manager/) — The installed manager.

### newWebRequest()

`protected function newWebRequest(array<string, mixed> $parameters = [], array<string> $additionalWhitelist = []): WebRequest`

Convenience factory for PSR-compatible WebRequest instances in tests.

additional parameter keys to whitelist.

| Parameter | Type | Description |
|---|---|---|
| `$parameters` | `array``<``string``, ``mixed``>` | runtime parameters to seed. |
| `$additionalWhitelist` | `array``<``string``>` | additional parameter keys to whitelist. |
