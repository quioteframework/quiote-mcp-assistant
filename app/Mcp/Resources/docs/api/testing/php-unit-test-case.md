...
|---|---|
| Extends | `TestCase` |
| Uses | [`PHPUnitTestCaseMethods`](/api/testing/phpunit-test-case-methods/) |
| Since | `1.0.0` |
| Source | `Testing/PhpUnitTestCase.php` |

## Properties

| Property | Type | Description |
|---|---|---|
| `$clearIsolationCache` | `mixed` | _protected._ |
| `$isolationDefaultContext` | `mixed` | _protected._ |
| `$isolationEnvironment` | `mixed` | _protected._ |
...
| [`doBootstrap(): boolean`](#dobootstrap) | Whether or not an quiote bootstrap should be done in isolation. |
...
| [`getClearCache(): bool`](#getclearcache) | check whether to clear the cache in isolated tests |
| [`getIsolationDefaultContext(): ?string`](#getisolationdefaultcontext) | get the default context to use in isolated tests |
| [`getIsolationEnvironment(): ?string`](#getisolationenvironment) | get the environment to bootstrap in isolated tests |
| [`setClearCache(bool $flag): void`](#setclearcache) | set whether the cache should be cleared for the isolated subprocess |
| [`setIsolationDefaultContext(string $contextName): void`](#setisolationdefaultcontext) | set the default context to use in isolated tests |
| [`setIsolationEnvironment(string $environmentName): void`](#setisolationenvironment) | set the environment to bootstrap in isolated tests |
| [`setUp(): void`](#setup) | Set up the test environment. |
| [`tearDown(): void`](#teardown) | Clean up after the test has run |

### createAttribute()

`public function createAttribute(string $name, mixed $value = null): void`

Composed in from [`PHPUnitTestCaseMethods`](/api/testing/phpunit-test-case-methods/).
...
`protected function doBootstrap(): boolean`

Whether or not an quiote bootstrap should be done in isolation.

Returns `boolean` — true if quiote should be bootstrapped
...
Returns `bool` — true if the cache is cleared in isolated tests

### getIsolationDefaultContext()

`public function getIsolationDefaultContext(): ?string`

get the default context to use in isolated tests

Returns `?``string` — the default context to use in isolated tests, or null if none is set

### getIsolationEnvironment()

`public function getIsolationEnvironment(): ?string`

get the environment to bootstrap in isolated tests

Returns `?``string` — the name of the isolation environment, or null if none is set

### setClearCache()

`public function setClearCache(bool $flag): void`
...
| Parameter | Type | Description |
|---|---|---|
| `$flag` | `bool` | true if the cache should be cleared |

### setIsolationDefaultContext()

`public function setIsolationDefaultContext(string $contextName): void`
...
### setIsolationEnvironment()

`public function setIsolationEnvironment(string $environmentName): void`
...
If running with isolation attributes, bootstrap Quiote with the specified environment.
