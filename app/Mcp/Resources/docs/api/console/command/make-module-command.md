...
> Scaffolds an empty module (Actions/Views/Templates directories) -- a module has no class of its own, it's a directory convention (see `ModuleActionDiscovery`), so unlike the other `make:*` commands this one has nothing to templatize beyond the directories themselves and an optional seed Action via --with-index.
...
| Since | `1.0.0` |
| Source | `Console/Command/MakeModuleCommand.php` |
