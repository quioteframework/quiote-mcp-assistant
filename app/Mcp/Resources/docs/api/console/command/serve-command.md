...
> Dev-server convenience wrapper: `quiote new`'s own \"next steps\" just tell the user to run `php -S` or `frankenphp php-server` by hand -- this runs whichever is available, and `--runtime` reaches the CLI-hosted servers too so there is one entry point for every deployment shape Quiote supports.

Dev-server convenience wrapper: `quiote new`'s own "next steps" just tell the user to run `php -S` or `frankenphp php-server` by hand -- this runs whichever is available, and `--runtime` reaches the CLI-hosted servers too so there is one entry point for every deployment shape Quiote supports.

Deliberately does not call [`AbstractAppCommand::bootstrapApp()`](/api/console/command/abstract-app-command/#bootstrapapp) (like NewCommand, it only needs to locate pub/, not boot the app in this process) -- it only needs [`AppDirResolver`](/api/console/app-dir-resolver/) for the same --app-dir/ $QUIOTE_APP_DIR/marker-file resolution every other command uses.
...
| Since | `1.0.0` |
| Source | `Console/Command/ServeCommand.php` |

## Constants

| Constant | Value | Description |
|---|---|---|
| `RUNTIMES` | `['auto', 'frankenphp', 'php', …]` |  |
| `RUNTIME_AUTO` | `'auto'` |  |
| `RUNTIME_FRANKENPHP` | `'frankenphp'` |  |
| `RUNTIME_PHP` | `'php'` |  |
| `RUNTIME_ROADRUNNER` | `'roadrunner'` |  |
| `RUNTIME_SWOOLE` | `'swoole'` |  |
