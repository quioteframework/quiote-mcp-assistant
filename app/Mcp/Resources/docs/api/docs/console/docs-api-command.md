...
The site is built in a repository that has no PHP, so the pages have to be generated here and committed there. `--check` is the other half of that arrangement: it compares what the current source would produce against what is committed, without writing, so a public API change that nobody regenerated fails a build rather than going unnoticed.
...
|---|---|
| Extends | `Command` |
| Source | `Console/DocsApiCommand.php` |
