...
try {
    $result = match ($capability) {
        ...
        'list_failed_requests' => \QuioteMcpAssistant\Mcp\Introspection\Capabilities\ListFailedRequests::run(
            $contextName,
            isset($options['since']) ? (string) $options['since'] : null,
            isset($options['limit']) ? (int) $options['limit'] : null,
        ),
        'describe_cassette' => \QuioteMcpAssistant\Mcp\Introspection\Capabilities\DescribeCassette::run(
            $contextName,
            (string) ($options['id'] ?? ''),
            in_array($options['include-bodies'] ?? '0', ['1', 'true'], true),
            isset($options['section']) ? (string) $options['section'] : null,
        ...
        'emit_replay_test' => \QuioteMcpAssistant\Mcp\Introspection\Capabilities\EmitReplayTest::run(
            $contextName,
            (string) ($options['id'] ?? ''),
            in_array($options['expect-fixed'] ?? '0', ['1', 'true'], true),
            isset($options['key']) ? (string) $options['key'] : null,
            isset($options['date']) ? (string) $options['date'] : null,
            isset($options['hour']) ? (string) $options['hour'] : null,
        ),
